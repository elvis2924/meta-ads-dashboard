...
<body class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <select id="tableResponsibleFilter" class="glass rounded-lg px-4 py-2 text-white outline-none w-full">
                        <option value="ericka" selected>Ericka</option>
                        <option value="pablo">Pablo</option>
                    </select>
                </div>
                <div>
    ...
    <script>
        ...
        // Gráfico de Tendencia - Tooltip solo en barra morada
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                const inversion = db.evolution.inversion[index];
                                const cpa = db.evolution.cpa[index];
                                return [
                                    'Inversión (S/): S/ ' + inversion.toFixed(2),
                                    'CPA (S/): S/ ' + cpa.toFixed(2)
                                ];
                            }
                        }
                    }
...
print(f"[SUCCESS] Dashboard corregido generado: {output_path}")
print(f"Tamano del archivo: {len(html_template):,} bytes")
print("Correcciones aplicadas:")
print("  - Excel se descarga correctamente como .xlsx (no ejecutable)")
print("  - Opcion 'Global' eliminada de filtro tabla (solo Ericka y Pablo)")
print("  - Tooltip solo en barra morada mostrando Inversion + CPA juntos")
